...
// thanks meteor for being epic and doing this shit for me
...
public class PlayerListEntryArgumentType implements ArgumentType<PlayerListEntry> {
    ...
    public PlayerListEntry parse(StringReader reader) throws CommandSyntaxException {
        ...
        for (PlayerListEntry p : SmolhudClient.Companion.getClient().getNetworkHandler().getPlayerList()) {
    ...
    public <S> CompletableFuture<Suggestions> listSuggestions(CommandContext<S> context, SuggestionsBuilder builder) {
        return CommandSource.suggestMatching(SmolhudClient.Companion.getClient().getNetworkHandler().getPlayerList().stream().map(playerListEntry -> playerListEntry.getProfile().getName()), builder);
    }
